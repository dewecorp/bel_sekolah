<?php
/**
 * Bootstrap aplikasi: autoload, helper global, settings
 */
...
class App
{
    ...
    /** Ikon Heroicons (Tailwind) - outline 24x24 */
    private const ICONS = [
        ...
        'pencil' => '<path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.35.837l-.888.296a.375.375 0 01-.41-.11l-2.8-2.8a.375.375 0 01-.1-.41l.296-.888a4.5 4.5 0 01.837-1.35l7.287-7.287.25-.25z"/><path stroke-linecap="round" stroke-linejoin="round" d="M20.571 9.943l-3.857-3.857"/>',
        'trash' => '<path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.055.68-.11 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"/>',
        ...
        'speaker' => '<path stroke-linecap="round" stroke-linejoin="round" d="M19.114 5.636a9 9 0 010 12.728M16.463 8.288a5.25 5.25 0 010 7.424M6.75 8.25l4.72-4.72a.75.75 0 011.28.53v15.88a.75.75 0 01-1.28.53l-4.72-4.72H4.51c-.88 0-1.704-.507-1.938-1.354A9.01 9.01 0 012.25 12c0-.83.112-1.633.322-2.396C2.806 8.756 3.63 8.25 4.51 8.25H6.75z"/>',
        ...
        'eye' => '<path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.573 16.49 16.638 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>',
        ...
        'clock-25' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/>',
    ...
    public static function icon(string $name, string $class = 'w-5 h-5', bool $stroke = true): string
    {
        $path = self::ICONS[$name] ?? '<path stroke-linecap="round" stroke-linejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z"/>';
        return '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" class="' . htmlspecialchars($class, ENT_QUOTES) . '">' . $path . '</svg>';
    }

    public static function init(): void
    {
        spl_autoload_register(function (string $class) {
            // Class di namespace Core -> core/
            if (strpos($class, 'Core\\') === 0) {
                $file = __DIR__ . '/' . str_replace('Core\\', '', $class) . '.php';
                if (file_exists($file)) {
                    require $file;
                }
                return;
            ...
            // Class di namespace App\Models -> models/
            if (strpos($class, 'App\\Models\\') === 0) {
                $file = __DIR__ . '/../models/' . str_replace('App\\Models\\', '', $class) . '.php';
                if (file_exists($file)) {
                    require $file;
                }
                return;
            ...
            // Class di namespace App\Controllers -> controllers/
            if (strpos($class, 'App\\Controllers\\') === 0) {
                $file = __DIR__ . '/../controllers/' . str_replace('App\\Controllers\\', '', $class) . '.php';
                if (file_exists($file)) {
                    require $file;
                }
                return;
    ...
    public static function isInstalled(): bool
    {
        try {
            ...
            $tables = Database::fetchAll('SHOW TABLES LIKE "settings"');
            if (empty($tables)) {
                return false;
            }
            $row = Database::fetch('SELECT COUNT(*) as c FROM settings');
    ...
    public static function settings(): array
    {
        ...
        $default = [
            'id'              => 1,
            'school_name'     => 'Sekolah Digital Indonesia',
            'school_logo'     => null,
            'school_address'  => '',
        ...
        try {
            $row = Database::fetch('SELECT * FROM settings WHERE id = 1');
            self::$settingsCache = $row ? array_merge($default, $row) : $default;
        } catch (\Throwable $e) {
            self::$settingsCache = $default;
        }
    ...
    {
        $logo = self::settings()['school_logo'] ?? null;
        if (empty($logo)) {
            return null;
        }
        return self::url($logo);
    }
    ...
    public static function now(string $timezone = 'Asia/Jakarta'): \DateTime
    {
        return new \DateTime('now', new \DateTimeZone($timezone));
    }

    public static function todayDate(string $timezone = 'Asia/Jakarta'): string
    {
        return self::now($timezone)->format('Y-m-d');
    }

    public static function todayTime(string $timezone = 'Asia/Jakarta'): string
    {
        return self::now($timezone)->format('H:i');
    }

    public static function dayName(string $timezone = 'Asia/Jakarta'): string
    {
        $days = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
        return $days[(int) self::now($timezone)->format('w')];
    }
